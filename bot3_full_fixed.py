...
class Logger:
    ...
    @staticmethod
    async def actionSuccess(msg): Logger.log("ACTION", "✅", msg, Colors.GREEN)
...
class KiteAi:
    ...
    async def get_web3_with_check(self, address: str, rpc_url: str, use_proxy: bool, retries=3, timeout=60):
        ...
        # PATCH class to handle extraData issue in Web3 v7+
...
def get_token_balance(self, address: str, rpc_url: str, contract_address: str, token_type: str, use_proxy: bool):
        try:
    ...
    async def perform_swap(self, account: str, address: str, rpc_url: str, token_in: str, amount: float, use_proxy: bool):
        try:
            ...
            # Check if token_in is USDT to get correct decimals
            token_contract = web3.eth.contract(
            ...
            approve_tx = token_contract.functions.approve(self.SWAP_ROUTER, amount_to_wei).build_transaction({
                "from": address,
                "nonce": web3.eth.get_transaction_count(address, "pending"),
                "chainId": web3.eth.chain_id,
                "gas": 100000 # A reasonable gas limit for a simple approve
            })

            approve_tx_hash = await self.send_raw_transaction_with_retries(account, web3, approve_tx)
            await self.wait_for_receipt_with_retries(web3, approve_tx_hash)
            logger.info(f"Approved {amount} token for swap.")

            # Create an empty instructions tuple with the correct structure
            # This is a workaround for the 'ABI not found' error
            empty_instructions = (
                0, # sourceId
                "0x0000000000000000000000000000000000000000", # receiver
                False, # payableReceiver
                "0x0000000000000000000000000000000000000000", # rollbackReceiver
    ...
    def print_swap_question(self):
        ...
        while True:
            try:
                min_swap_amount = float(input(f"{Colors.YELLOW + Colors.BOLD}Min Swap Amount? -> {Colors.RESET}").strip())
        ...
        while True:
            try:
                max_swap_amount = float(input(f"{Colors.YELLOW + Colors.BOLD}Max Swap Amount? -> {Colors.RESET}").strip())
    ...
    async def process_option_7(self, account: str, address: str, use_proxy: bool):
        ...
        for i in range(self.swap_count):
            ...
            logger.info(f"    Pair    : {token_in_name}->{token_out}")
            logger.info(f"    Balance : {balance} {token_in_name}")
            logger.info(f"    Amount  : {amount} {token_in_name}")

            if not balance or balance <= amount:
                logger.warn(f"Insufficient {token_in_name} Token Balance. Skipping swap.")
                continue
